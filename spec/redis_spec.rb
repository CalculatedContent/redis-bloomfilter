...
describe RedisBloomFilter::Redis do

  context "use Redis bitstring for storage" do
    let(:bf) { RedisBloomFilter::Redis.new }
    ...
    it "should connect to remote redis server" do
      lambda { RedisBloomFilter::Redis.new }.should_not raise_error
    end

    it "should allow redis client instance to be passed in" do
      redis_client = mock ::Redis
      bf = RedisBloomFilter::Redis.new(:db => redis_client)
      bf.instance_variable_get(:@db).should be == redis_client
    end

    it "should allow namespaced BloomFilters" do
      bf1 = RedisBloomFilter::Redis.new(:namespace => :a)
      bf2 = RedisBloomFilter::Redis.new(:namespace => :b)
